...
def edit_user_record(db, user_id, username, password, email, contact):
    db = create_connection()  
    ...
    try:
        ...
        st.error(f"❌ Failed to update user record: {e}")
    finally:
        cursor.close()
...
""", unsafe_allow_html=True)
            ...
            username = st.text_input(":orange[username]", value=user['username'])
            email = st.text_input(":orange[Email]", value=user['email'])
            contact = st.text_input(":orange[Contact]", value=user['contact'])
            if st.session_state.show_password_fields:
                old_password = st.text_input(":orange[Old Password]", type="password")
            ...
            if update:
                ...
                st.session_state.edit_user.update({
                    'username': username,
                    'email': email,
                    'contact': contact,
                })
...
def search_edit_and_update_user(db, username):
    if username:
        ...
        if user:
            ...
            st.session_state.edit_user = user_dict
            edit_user(db)
        else:
...
def user_observations_page():
    if st.button("🔙 Menu"):
        ...
        st.rerun()
    parents_observations.main()

def user_appintments_page():
    if st.button("🔙 Menu"):
        st.session_state.user_action = None
        st.rerun()
    parents_tool_page.main()
