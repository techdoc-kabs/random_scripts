...
def dynamic_cumulative_multiselect_filter(df):
    ...
    filterable_columns = [
        'client_type', 'screen_type', 'term', 'class', 'stream',
        'Depression Status', 'Anxiety Status', 'Suicide Risk', 'Functioning'
    ]
    available_filter_columns = [col for col in filterable_columns if col in df.columns]
    columns_to_filter = st.sidebar.multiselect("Choose columns to filter", options=available_filter_columns)
    filtered_df = df.copy()
    for col in columns_to_filter:
        unique_vals = filtered_df[col].dropna().unique()
        selected_vals = st.sidebar.multiselect(f"Select values for {col}", options=sorted(unique_vals))
        if selected_vals:
...
def generate_combined_summary_table(db, return_unstyled=False):
    ...
    cursor.execute("""
        SELECT appointment_id, name, screen_type, class, stream, term, client_type, 
               screening_tools, tools_response_dates, actions
        FROM appointments
    ...
    for row in rows:
        try:
            ...
            if not actions.get("screen", False):  # Skip if screen not true
                continue
        ...
        try:
            ...
            st.warning(f"Failed to parse tools_response_dates for {appointment_id}: {e}")
...
def generate_combined_summary_data(db, return_unstyled=False):
    ...
    # Calendar-based filtering by year and month
    ...
    if valid_date_col:
        ...
        df["Month"] = df["response_date"].dt.strftime('%B')  # e.g. "January", "February"
...
def main():
    ...
    if grouped_dfs:
        ...
        st.info("⚠️ No grouped data to include in the CSV file. Please check your filters or selection.")
