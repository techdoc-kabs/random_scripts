...
def get_pushbullet():
    try:
        pb_instance = Pushbullet(API_KEY)  # Will try to connect to API
        return pb_instance
    except requests.exceptions.ConnectionError:
        st.warning("⚠ Pushbullet is offline. Internet connection not available.")
        return None
    except Exception as e:
        st.error(f"Pushbullet failed to initialize: {e}")
        return None

pb = get_pushbullet()

# --- Safe Pushbullet Send ---
def safe_push(title, body):
    """Send Pushbullet notification but don't crash if offline."""
    try:
        if pb is not None:
            pb.push_note(title, body)
    except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
        st.warning("⚠️ Could not send notification due to poor internet connection.")
    except Exception as e:
...
def show_login_dialog():
    with st.form("login_form"):
        ...
        if submitted:
            success, username, role = authenticate_user(username, password)  # ✅ always 3 values
            if success:
    ...
    with create_connection() as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                role TEXT,
                name TEXT,
                event_type TEXT CHECK(event_type IN ('login', 'signup', 'logout')) NOT NULL,
                timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                session_duration INTEGER -- duration in seconds, only for logout event
            )
        """)
...
#                         insert_session_event(user_id,role, name, 'login')

#                     # Extract column names for creating user dict
#                     col_names = [desc[1] for desc in conn.execute("PRAGMA table_info(users)")]
#                 user = dict(zip(col_names, row)) if row else {}
...
def show_signup_dialog():
    with st.form("signup_form"):
        ...
        role = col2.selectbox("I am a...", ["Select", "Student", "Parent", "Teacher"])
        if role == "Student":
        ...
        if submitted:
            ...
            if missing:
                ...
                if ok:
                    insert_session_event(user_data['user_id'],user_data['full_name'],user_data['role'], 'signup')
                    st.success(msg)
                    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    safe_push(
