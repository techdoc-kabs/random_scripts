...
# def teachers_observations():
#     conn = create_connection()
#     c = conn.cursor()
#     c.execute("""
#         CREATE TABLE IF NOT EXISTS teachers_observations (
...
#     c = conn.cursor()
#     c.execute("""
#         INSERT INTO teachers_observations (
...
# def main():
#     teachers_observations()
#     update_teachers_observations_schema()
#     class_options = get_class_options()
#     stream_options = get_stream_options()
#     username = st.session_state.get("user_name")
#     teachers_name = get_full_name_from_username(username)
...
#         with st.sidebar.expander('REPORT ISSUES', expanded=True):
#             observation_type = st.radio("Observation Type", ["Individual Student", "Whole Class/Stream"])
...
#         if observation_type == "Individual Student":
#             st.markdown("### REPORT INDIVIDUAL STUDENT")
...
#                         "Observed Behavior", [
#                             "Withdrawal from peers", "Drop in academic performance",
...
#                             "Stress from home", "Peer conflicts", "Bullying",
#                             "Learning difficulties", "Unknown", "Other (specify)"
#                         ])
#                     recommendation = multiselect_with_other(
#                         "Recommended Action", [
#                             "Refer to counselor", "Monitor behavior",
#                             "Immediate parental involvement", "Other (specify)"
...
#                         ))
#                         st.success(f"✅ Thank you teacher {teachers_name} for your submission!")

#         elif observation_type == "Whole Class/Stream":
#             st.markdown(" ### REPORT ISSUES ON CLASS/STREAM")
#             with st.expander('OBSERVATION FORM', expanded=True):
#                 col1, col2 = st.columns([1, 2])
#                 with col1:
#                     class_name = multiselect_with_other("Class", class_options)
#                     stream_name = multiselect_with_other("Stream", stream_options)
#                     date_observed = st.date_input("Date of Observation", value=date.today())
#                     observed_behavior = multiselect_with_other(
#                     "Observed Behavior", [
#                         "Withdrawal from peers", "Drop in academic performance",
...
#                     ])
#                 with col2:
#                     description = st.text_area("Please describe the observation (class/stream-wide)")
#                     possible_cause = multiselect_with_other(
#                         "Possible Cause / Trigger", [
#                             "Stress from home", "Peer conflicts", "Bullying",
#                             "Learning difficulties", "Unknown", "Other (specify)"
...
#                     urgency = st.radio("Urgency Level", ["Low", "Moderate", "High"], horizontal=True)
#                 submitted_by = teachers_name
...
# ---------------- Users Table Safe Queries ----------------
def get_class_options():
    conn = create_connection()
    if not conn or not table_exists(conn, "users"):
        return ["Other (specify)"]
    try:
        cur = conn.cursor()
        cur.execute("SELECT DISTINCT class FROM users WHERE class IS NOT NULL AND class != ''")
...
def fetch_users(db, search_input):
    """Safely fetch users; return empty list if table missing or query fails."""
    cursor = db.cursor()
    try:
...
def get_full_name_from_username(username):
    ...
    try:
        ...
        cursor.execute("SELECT full_name FROM users WHERE username = ?", (username,))
        result = cursor.fetchone()
        return result["full_name"] if result else username
    except sqlite3.Error:
        return username
    finally:
        conn.close()

# ---------------- Teachers Observations Table ----------------
def teachers_observations():
    conn = create_connection()
    if not conn:
        return
